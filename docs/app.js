...
// include: /var/folders/v5/848k4ql56ps1xwrmtm4_wn0c0000gn/T/tmp46ied5jn.js
...
Module['FS_createPath']("/data", "shaders", true, true);
    ...
    loadPackage({"files": [{"filename": "/data/shaders/blit.wgsl", "start": 0, "end": 576}, {"filename": "/data/shaders/gradient.wgsl", "start": 576, "end": 1383}, {"filename": "/data/shaders/terrain.wgsl", "start": 1383, "end": 4978}], "remote_package_size": 4978});

  })();

// end include: /var/folders/v5/848k4ql56ps1xwrmtm4_wn0c0000gn/T/tmp46ied5jn.js
// include: /var/folders/v5/848k4ql56ps1xwrmtm4_wn0c0000gn/T/tmp462vu_m7.js

    // All the pre-js content up to here must remain later on, we need to run
    // it.
    if (Module['$ww'] || (typeof ENVIRONMENT_IS_PTHREAD != 'undefined' && ENVIRONMENT_IS_PTHREAD)) Module['preRun'] = [];
    var necessaryPreJSTasks = Module['preRun'].slice();
  // end include: /var/folders/v5/848k4ql56ps1xwrmtm4_wn0c0000gn/T/tmp462vu_m7.js
// include: /var/folders/v5/848k4ql56ps1xwrmtm4_wn0c0000gn/T/tmpcnmq3d39.js

    if (!Module['preRun']) throw 'Module.preRun should exist because file support used it; did a pre-js delete it?';
    necessaryPreJSTasks.forEach((task) => {
      if (Module['preRun'].indexOf(task) < 0) throw 'All preRun tasks that exist before user pre-js code should remain after; did you replace Module or modify Module.preRun?';
    });
  // end include: /var/folders/v5/848k4ql56ps1xwrmtm4_wn0c0000gn/T/tmpcnmq3d39.js
...
function update_fps_overlay(fps) { let el = document.getElementById('fps-counter'); if (!el) { el = document.createElement('div'); el.id = 'fps-counter'; el.style.position = 'absolute'; el.style.top = '8px'; el.style.left = '8px'; el.style.padding = '4px 8px'; el.style.background = 'rgba(0, 0, 0, 0.55)'; el.style.color = '#0f0'; el.style.font = '12px monospace'; el.style.borderRadius = '4px'; el.style.zIndex = '1000'; document.body.appendChild(el); } el.textContent = 'FPS: ' + fps.toFixed(1); }
function canvas_prefers_bgra8unorm() { if (navigator.gpu && navigator.gpu.getPreferredCanvasFormat) { return navigator.gpu.getPreferredCanvasFormat() === 'bgra8unorm'; } return 1; }
...
// end include: preamble.js
  ...
  fillLimitStruct:(limits, supportedLimitsOutPtr) => {
        ...
        function setLimitValueU64(name, limitOffset) {
          var limitValue = limits[name];
          (tempI64 = [limitValue>>>0,(tempDouble = limitValue,(+(Math.abs(tempDouble))) >= 1.0 ? (tempDouble > 0.0 ? (+(Math.floor((tempDouble)/4294967296.0)))>>>0 : (~~((+(Math.ceil((tempDouble - +(((~~(tempDouble)))>>>0))/4294967296.0)))))>>>0) : 0)], HEAP32[(((limitsOutPtr)+(limitOffset))>>2)] = tempI64[0],HEAP32[(((limitsOutPtr)+((limitOffset)+(4)))>>2)] = tempI64[1]);checkInt64(limitValue);
        }
  ...
  var _wgpuDevicePopErrorScope = (deviceId, callback, userdata) => {
      var device = WebGPU.mgrDevice.get(deviceId);
      ...
      device.popErrorScope().then((gpuError) => {
        ...
        callUserCallback(() => {
          if (!gpuError) {
            ...
            WebGPU.errorCallback(callback, 1, gpuError.message, userdata);
          }
        });
      }, (ex) => {
        ...
        callUserCallback(() => {
          ...
          WebGPU.errorCallback(callback, 4, ex.message, userdata);
        });
      });
    };

  var _wgpuDevicePushErrorScope = (deviceId, filter) => {
      var device = WebGPU.mgrDevice.get(deviceId);
...
var wasmImports = {
  ...
  /** @export */
  wgpuDevicePopErrorScope: _wgpuDevicePopErrorScope,
  /** @export */
